...
def answer_four():
    ...
    # 2006 컬럼값 - 2015 컬럼값
    return abs(
        df2.iloc[5, df2.columns.get_loc("2006")]
        - df2.iloc[5, df2.columns.get_loc("2015")]
    )
...
def answer_nine():
    ...
    return df["Citable documents per person"].corr(
...
def answer_ten():
    ...
    # 'Rank' 오름 차순으로 'HeightRenew' 컬럼 출력
...
def get_df_continent_top15():
    ...
    df_top15 = answer_one()
    df_top15["estimate"] = (
        df_top15["Energy Supply"] / df_top15["Energy Supply per Capita"]
    )
    ContinentDict = {
        "China": "Asia",
...
def answer_eleven():
    ...
    df["size"] = df.groupby([df.index])["Country"].count()
    # estimante 합
    df["sum"] = df.groupby([df.index])["estimate"].sum()
    # estimante 평균
    df["mean"] = df.groupby([df.index])["estimate"].mean()
    # estimante 표준편차(standard deviation)
    # ddof(Delta Degrees of Freedom) 자유도(N-1)
    # 표준편차 계산시 분모(n-1, n 표본 개수)에서 1(디폴트) 대신 0설정
    df["std"] = df.groupby([df.index])["estimate"].std(ddof=0)
...
def answer_twelve():
    ...
    df["country count"] = df.groupby(level=["Continent", "bins"])["Country"].count()
