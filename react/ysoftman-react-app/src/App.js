...
function Create(props) {
  return <article>
    ...
    <form onSubmit={event => {
      ...
      <p><textarea name="cost" placeholder='cost'></textarea></p>
...
function Update(props) {
  ...
  return <article>
    ...
    <form onSubmit={event => {
      ...
      <p><textarea name="cost" placeholder='cost' value={cost} onChange={event => {
        // console.log("event:", event.target.value)
...
function MyContents(props) {
  ...
  for (let i = 0; i < props.items.length; i++) {
    ...
    mylist.push(<li key={l.id}>
      <a id={l.id} name={l.name} href="/" onClick={(event) => {
...
function App() {
  ...
  if (mode === "read") {
    ...
    updateDeleteComponent = <>
      <li><a href={'/' + click_id} onClick={event => {
        event.preventDefault()
        setMode("update")
      }}>upate</a></li>
    ...
    message = <Update name={_name} cost={_cost} onUpdate={(_name, _cost) => {
      const updatedItem = { id: Number(click_id), name: _name, cost: _cost }
      // 기존 mylist 복제
      const newList = [...mylist]
  ...
  return <div >
    ...
    <MyContents items={mylist} onChangeMode222={(id, name) => {
