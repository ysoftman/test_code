...
func main() {
	...
	go func() {
		fmt.Println("http url: http://localhost/main")
	...
	// https 로 띄우기(Go 1.6 부터 net/http  에서 http2 지원)
	fmt.Println("https url: https://localhost/main")
	log.Fatal(http.ListenAndServeTLS(":443", "cert/server.crt", "cert/server.key", nil))
}
