...
func main() {
	...
	go sendValue1(channel1, &wg)
	go sendValue2(channel2, &wg)
	go recvValue(channel1, channel2, &wg)
	//  WaitGroup 의 고루틴들이 모두 끝날(Done)때까지 대기
	wg.Wait()
}

func sendValue1(ch chan int, wg *sync.WaitGroup) int {
	defer wg.Done()
	value := 0
	for {
		time.Sleep(100 * time.Millisecond)
		fmt.Printf("[%s] sendValue1 [%d]\n", time.Now().Format("2006-01-02 15:04:05"), value)
	...
	fmt.Printf("[%s] sendValue1 end\n", time.Now().Format("2006-01-02 15:04:05"))
	return 0
}

func sendValue2(ch chan int, wg *sync.WaitGroup) int {
	defer wg.Done()
	value := 0
	for {
		time.Sleep(200 * time.Millisecond)
		fmt.Printf("[%s] sendValue2 \t[%d]\n", time.Now().Format("2006-01-02 15:04:05"), value)
	...
	fmt.Printf("[%s] sendValue2 \tend\n", time.Now().Format("2006-01-02 15:04:05"))
	return 0
}

func recvValue(ch1, ch2 chan int, wg *sync.WaitGroup) {
	defer wg.Done()
	cnt1, cnt2 := 0, 0
	for {
