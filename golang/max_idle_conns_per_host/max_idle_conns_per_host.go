...
func main() {
	...
	for i := 0; i < 10; i++ {
		...
		// 프로그램이 끝나면 10개는 time_wait 로 되어 있는건 프로그램이 끝났으니 사용되지 않아 time_wait 상태로가 유지되고, 60초(커널에 하드코딩된 time_wait 만료시간) 후 사라진다.
...
func startCustomClientReq(jobNum int) {
	...
	for {
		resp, err := myClient.Get("http://localhost:55555")
		if err != nil {
			log.Println(err.Error())
		}
		// resp.Body 를 읽지 못해서 time_wait 가 발생하는 경우 resp.Body 를 무시하도록 한다.
