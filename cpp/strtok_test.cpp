...
// 여기서 한가지 의문의 생긴다... 입력문자열에 NULL 값을 집어넣는데 어떻게 토큰을 계속 찾을 수
// 있을까? strtok() 내부에서는 토큰을 찾고 찾은 토큰 바로 뒤의 문자를 NULL 로 만들어 버린다. 즉 입력
// 문자열이 되어 strtok() 사용전 입력문자열를 복사해두고 사용해아 한다. 처음 strtok() 를 호출할 때
// strtok() 함수 내부에서 첫번째 토큰을 리턴하기 전 NULL 다음부터의 문자열을 기억하고 있는다. 두번째
// strtok() 를 호출할 때 입력문자열 대신 NULL 값을 주면 바로 이전에 사용되었던 문자열을 끄집어내서
// 사용하게 된다.
