...
char *UTF8ToANSI(char *pUTF8) {
    ...
    char *pResult = NULL;
#if defined(_WIN32) || defined(_WIN64)
    // CP_ACP (current Windows ANSI code page) 는 영어 윈도우에서는 ascii, 한글 윈도우에서는 euc-kr,
    // 일본 윈도우에서는 euc-jp 로 작동 인코딩 방법 : utf-8(멀티바이트) -> 유니코드 -> ANSI(euc-kr,
    // euc-kr...) (멀티바이트) multibyte UTF8 의 크기 파악
...
#if defined(_WIN32) || defined(_WIN64)
    // CP_ACP (current Windows ANSI code page) 는 영어 윈도우에서는 ascii, 한글 윈도우에서는 euc-kr,
    // 일본 윈도우에서는 euc-jp 로 작동 인코딩 방법 : ANSI(euc-kr, euc-kr...) (멀티바이트) ->
    // 유니코드 -> utf-8(멀티바이트) multibyte ANSI(euc-kr, euc-kr...) 의 크기 파악
