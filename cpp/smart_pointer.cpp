...
// 스마트 포인터를 생성하면 rc(reference counter), wrc(weak reference counter)등을 관리하는 control block 이 생긴다.
#include <iostream>
#include <string>
...
void shared_pointer_test()
{
    ...
    // 소유권이 공유되면 rc(reference counter) 가 증가되며, rc 가 0 되면 포인팅 되는 대상 메모리 해제
...
void weak_pointer_test()
{
    ...
    std::shared_ptr<string> sp1 = std::make_shared<string>("ysoftman");
    // weak_ptr 는 shared_ptr 를 참조 하는데, 메모리 해제를 관여하는 rc 는 증가시키지는 않고 wrc(weak reference counter)만 증가시킨다.
    // 2개의 shared_ptr 가 서로를 참조하는 순환 참조를 해야 될때, rc 는 0이 될 수 없어 메모리가 해제되지 않기 때문에, 이 경우 weak_ptr 를 사용한다.
