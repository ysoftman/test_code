...
int main()
{
	...
	// 레퍼런스 변수는 참조하는 변수의 alias 로 이름만 다를뿐 참조 하는 변수의 메모리 공간(주소)을 공유하기 때문에
	// 레퍼런스 변수를 자신을 위한 별도의 메모리 할당이 없다.
	// 포인터 변수는 포인터 변수 자신을 위한 별도의 메모리를 할당 받고
	// 포인터 변수의 값은 포인터 변수가 가리키는(참조하는) 변수의 주소값을 저장하한다.
