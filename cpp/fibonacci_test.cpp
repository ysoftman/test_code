...
// 재귀함수 이용(이전에 계산된 값을 저장해 중복 계산을 막는다.) 2*N => 시간복잡도 표시에 상수는 제외
// 해서 => O(N)
int fibonacci_recursive_with_array(int n, int *arr) {
    ...
    result =
        fibonacci_recursive_with_array(n - 1, arr) + fibonacci_recursive_with_array(n - 2, arr);
    arr[n] = result;
    return result;
}
...
int fibonacci_iteration(int n) {
    ...
    for (i = 3; i <= n; i++) {
        debug_count++;
        result = temp1 + temp2;
        // 이전 값을 저장해 다음번에 계산없이 사용할 수 있도록 한다.(DynamicProgramming,
        // memoization)
        temp1 = temp2;
        temp2 = result;
        if (result <= 0) {
